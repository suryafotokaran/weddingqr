      ...
      <nav className="fixed top-0 z-50 w-full px-8 py-4 flex justify-between items-center bg-white/80 backdrop-blur-xl shadow-[0_12px_40px_rgba(26,28,28,0.04)]">
        <div className="flex items-center gap-12">
          ...
          <div className="hidden md:flex items-center space-x-8 text-sm tracking-tight">
            <a className={`font-bold transition-colors ${location.pathname === '/studio' ? 'text-teal-700 border-b-2 border-teal-600' : 'text-zinc-500 hover:text-teal-600'}`} href="/studio">Dashboard</a>
        ...
        <aside className="hidden lg:flex flex-col w-64 h-[calc(100vh-6rem)] p-4 space-y-2 bg-zinc-50 sticky top-24">
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-transform duration-200 ${
                  isActive 
          ...
          <div className="mt-auto p-4 border-t border-zinc-200/50">
            <button
              ...
              className={`flex items-center gap-3 w-full px-4 py-3 rounded-xl text-sm font-medium transition-transform duration-200 ${
                location.pathname === '/profile'
                  ? 'bg-orange-100 text-orange-900 active:scale-[0.98]'
                  : 'text-zinc-600 hover:bg-zinc-200/50 hover:translate-x-1'
              }`}
            >
              <Settings size={20} className={location.pathname === '/profile' ? 'text-teal-700' : ''} />
              Settings
            </button>
          </div>
        </aside>
